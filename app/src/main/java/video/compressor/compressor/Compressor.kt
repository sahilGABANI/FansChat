package video.compressor.compressor
...
import video.compressor.CompressionProgressListener
import video.compressor.config.Configuration
import video.compressor.utils.CompressorUtils.findTrack
import video.compressor.utils.CompressorUtils.generateWidthAndHeight
import video.compressor.utils.CompressorUtils.getBitrate
import video.compressor.utils.CompressorUtils.hasQTI
import video.compressor.utils.CompressorUtils.prepareVideoHeight
import video.compressor.utils.CompressorUtils.prepareVideoWidth
import video.compressor.utils.CompressorUtils.printException
import video.compressor.utils.CompressorUtils.setOutputFileParameters
import video.compressor.utils.CompressorUtils.setUpMP4Movie
import video.compressor.utils.StreamableVideo
import video.compressor.video.InputSurface
import video.compressor.video.MP4Builder
import video.compressor.video.OutputSurface
import video.compressor.video.Result
import java.io.File
...
object Compressor {
    ...
    suspend fun compressVideo(
        ...
        try {
            ...
            return@withContext Result(index, success = false, failureMessage = "${exception.message}")
        }
        ...
        if (rotationData.isNullOrEmpty() || bitrateData.isNullOrEmpty() || durationData.isNullOrEmpty()) {
            // Exit execution
            return@withContext Result(index,
                success = false,
        ...
        if (configuration.isMinBitrateCheckEnabled && bitrate <= MIN_BITRATE) return@withContext Result(index,
            success = false,
            failureMessage = INVALID_BITRATE)

        //Handle new bitrate value
        val newBitrate: Int = if (configuration.videoBitrate == null) getBitrate(bitrate, configuration.quality)
        else configuration.videoBitrate!!

        //Handle new width and height values
        var (newWidth, newHeight) = if (configuration.videoHeight != null) Pair(configuration.videoWidth?.toInt(),
            configuration.videoHeight?.toInt())
        else generateWidthAndHeight(width, height, configuration.keepOriginalResolution)
        ...
        return@withContext start(index,
            newWidth!!,
            newHeight!!,
            destination,
            newBitrate,
    ...
    private fun start(id: Int,
        ...
        if (newWidth != 0 && newHeight != 0) {
            ...
            try {
                ...
                val outputFormat: MediaFormat = MediaFormat.createVideoFormat(MIME_TYPE, newWidth, newHeight)
                //set output format
                setOutputFileParameters(inputFormat, outputFormat, newBitrate, frameRate)
                ...
                try {
                    ...
                    while (!outputDone) {
                        ...
                        loop@ while (decoderOutputAvailable || encoderOutputAvailable) {
                            ...
                            if (!isRunning) {
                                dispose(videoIndex, decoder, encoder, inputSurface, outputSurface, extractor)

                                compressionProgressListener.onProgressCancelled(id)
                                return Result(id, success = false, failureMessage = "The compression has stopped!")
                            }

                            //Encoder
                            val encoderStatus = encoder.dequeueOutputBuffer(bufferInfo, MEDIACODEC_TIMEOUT_DEFAULT)

                            when {
                                encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER -> encoderOutputAvailable = false
                                encoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> {
                                    val newFormat = encoder.outputFormat
                                    if (videoTrackIndex == -5) videoTrackIndex = mediaMuxer.addTrack(newFormat, false)
                                }
                            ...
                            //Decoder
                            val decoderStatus = decoder.dequeueOutputBuffer(bufferInfo, MEDIACODEC_TIMEOUT_DEFAULT)
                            when {
                                ...
                                decoderStatus < 0 -> throw RuntimeException("unexpected result from decoder.dequeueOutputBuffer: $decoderStatus")
                                    ...
                                    if (doRender) {
                                        ...
                                        try {
                                            ...
                                            errorWait = true
                                            Log.e("Compressor", e.message ?: "Compression failed at swapping buffer")
                                        }
                                        ...
                                        if (!errorWait) {
                                            ...
                                            compressionProgressListener.onProgressChanged(id,
                ...
                processAudio(mediaMuxer = mediaMuxer, bufferInfo = bufferInfo, disableAudio = disableAudio, extractor)
            ...
            Timber.tag("Streamable : ").e(streamableFile.toString())
            Timber.tag("Destination : ").e(destination)
            Timber.tag("ResultFile : ").e(resultFile.path.toString())
            return Result(id, success = true, failureMessage = null, size = resultFile.length(), resultFile.path)
        }

        return Result(id, success = false, failureMessage = "Something went wrong, please try again")
    }

    private fun processAudio(mediaMuxer: MP4Builder,
    ...
    private fun dispose(videoIndex: Int,
        decoder: MediaCodec,
        encoder: MediaCodec,
