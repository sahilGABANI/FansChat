...
class VerticalSeekBarWrapper : FrameLayout {
    ...
    private fun onSizeChangedTraditionalRotation(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (seekBar != null) {
            ...
            seekBar.measure(MeasureSpec.makeMeasureSpec(max(0, w - hPadding), MeasureSpec.AT_MOST),
    ...
    private fun onSizeChangedUseViewRotation(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (seekBar != null) {
            val hPadding = paddingLeft + paddingRight
            val vPadding = paddingTop + paddingBottom
            seekBar.measure(MeasureSpec.makeMeasureSpec(max(0, h - vPadding), MeasureSpec.EXACTLY),
    ...
    private fun applyViewRotation(w: Int, h: Int) {
        ...
        if (seekBar != null) {
            ...
            val rotationAngle = seekBar.getRotationAngle()
            val seekBarMeasuredWidth = seekBar.measuredWidth
    ...
    private fun getChildSeekBar(): VerticalSeekBar? {
