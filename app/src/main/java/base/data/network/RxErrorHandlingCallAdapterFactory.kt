...
import base.data.network.NetworkException.Companion.asRetrofitException
                ...
                is Single<*> -> result.onErrorResumeNext(Function { throwable ->
                    Single.error(
                        asRetrofitException(
                            annotations,
                            retrofit,
                           if (throwable is IOException) Throwable("Please check your internet connection!") else throwable
                        )
                    )
                })
